...
import { 
  ...
  InformationCircleIcon, UsersIcon, CalendarDaysIcon, ChevronDoubleLeftIcon, ChevronDoubleRightIcon 
} from '../ui/icons';
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, toggleSidebar, isLgCollapsed, toggleLgCollapse }) => {
  ...
  const navLinkClass = ({ isActive }: { isActive: boolean }) =>
    `flex items-center px-3 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-sap-gray dark:hover:bg-gray-700 hover:text-sap-blue dark:hover:text-white rounded-md transition-colors duration-150 ${isLgCollapsed ? 'lg:justify-center' : ''} ${
  ...
  const iconClasses = `flex-shrink-0 w-5 h-5 ${isLgCollapsed ? 'lg:mr-0' : 'mr-3'}`;
  const textClasses = `whitespace-nowrap ${isLgCollapsed ? 'lg:hidden' : 'lg:block'}`;
    ...
    <>
      ...
      >
        ...
        <nav className="flex-grow py-4 px-2 overflow-y-auto">
          <ul>
             <li>
              <NavLink to={ROUTES.DASHBOARD} className={navLinkClass} onClick={handleLinkClick}>
        ...
        <div className="px-2 py-3 border-t border-sap-border dark:border-gray-700">
           <button
            onClick={toggleTheme}
            className={`flex items-center w-full px-3 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-sap-gray dark:hover:bg-gray-700 hover:text-sap-blue dark:hover:text-white rounded-md transition-colors duration-150 ${isLgCollapsed ? 'lg:justify-center' : ''}`}
